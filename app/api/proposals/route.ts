...
export async function POST(req: NextRequest) {
  try {
    ...
    // Prevent client from proposing on their own project
    if (project.clientId.toString() === userId) {
      return NextResponse.json({ error: "You cannot submit a proposal on your own project" }, { status: 400 })
    }
...
export async function GET(req: NextRequest) {
  try {
    ...
    const proposals = await Proposal.find(query)
      .populate("freelancerId", "name image username rating reviewCount skills hourlyRate location")
