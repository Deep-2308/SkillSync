...
import { Search, Filter, Star, MapPin, Zap, Loader2, DollarSign, Clock, ArrowRight } from "lucide-react"
...
export default function HireTalentPage() {
  ...
  const [projects, setProjects] = useState<Array<{_id: string; title: string; description: string; budget: number; duration: number; skills: string[]; status: string; category: string; createdAt: string}>>([])
          ...
          {projects.length > 0 && (
            <AnimatedSection animation="slideUp" delay={300}>
              <div className="mt-20 mb-10">
                <span className="font-mono text-[11px] tracking-[0.2em] uppercase text-primary/60 mb-3 block">Open Projects</span>
                <h2 className="font-display text-2xl md:text-3xl font-bold mb-2 tracking-tight">
                  Find <span className="text-primary">Work</span>
                </h2>
                <p className="font-serif text-muted-foreground italic mb-8">
                  Browse open projects and submit your proposal to get hired.
                </p>
                  ...
                  {projects.slice(0, 6).map((project, i) => (
                    <AnimatedSection key={project._id} animation="fadeIn" delay={i * 60}>
                      <Link href={`/projects/${project._id}`}>
                        <Card className="card-shimmer group cursor-pointer h-full flex flex-col">
                          <CardContent className="p-5 flex-1 flex flex-col">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="font-mono text-[8px] tracking-wider uppercase text-accent border border-accent/20 bg-accent/5 px-1.5 py-0.5 rounded">
                                {project.status}
                              </span>
                              {project.category && (
                                <span className="font-mono text-[8px] tracking-wider uppercase text-muted-foreground border border-border px-1.5 py-0.5 rounded">
                                  {project.category}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-1.5 mb-4">
                              {project.skills.slice(0, 4).map((s) => (
                                <span key={s} className="font-mono text-[8px] tracking-wider uppercase text-primary/60 border border-primary/10 px-1.5 py-0.5 rounded bg-primary/5">{s}</span>
                              ))}
                            </div>
                            <div className="mt-auto pt-3 border-t border-border/30 flex justify-between items-center">
                              <div className="flex gap-3 text-[10px] font-mono text-muted-foreground">
                                <span><DollarSign className="h-3 w-3 inline text-primary" /> ${project.budget.toLocaleString()}</span>
                                <span><Clock className="h-3 w-3 inline text-primary" /> {project.duration}d</span>
                              </div>
